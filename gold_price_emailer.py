...
# Optional: AI-written market commentary, using the Claude API directly
# (plain requests.post - no SDK dependency needed). Auto-enables when
# ANTHROPIC_API_KEY is set; leave it unset to skip this section entirely
# with zero cost/behavior change otherwise. Defaults to Haiku (cheapest,
# plenty good for a short factual summary) - set COMMENTARY_MODEL to
# "claude-sonnet-5" for richer prose at higher cost per run.
ANTHROPIC_API_KEY = os.environ.get("ANTHROPIC_API_KEY", "")
COMMENTARY_MODEL = os.environ.get("COMMENTARY_MODEL") or "claude-haiku-4-5-20251001"
COMMENTARY_ENABLED = bool(ANTHROPIC_API_KEY)
...
def build_commentary_input(summary_tables, silver, world_gold, gap_rows, big_moves, extremes):
    """
    Compact JSON-safe snapshot of the day's key figures - the only data
    generate_commentary is allowed to reference, so the AI-written
    paragraph stays grounded in real numbers instead of inventing
    context. Keeping this small also keeps the API call cheap.
    """
    data = {}
    if summary_tables:
        sjc = next((r for r in summary_tables[0] if r["label"] == "SJC"), None)
        if sjc:
            data["vang_sjc_ban_ra"] = sjc["sell"]
            data["vang_sjc_mua_vao"] = sjc["buy"]
    if "data" in world_gold:
        d = world_gold["data"]
        data["gia_the_gioi_usd_oz"] = round(d["xau_usd"], 2)
        data["bien_dong_the_gioi_24h_pct"] = round(d["change_pct"], 2)
    if gap_rows:
        sjc_gap = next((r for r in gap_rows if r["label"] == "SJC"), gap_rows[0])
        data["chenh_lech_trong_nuoc_the_gioi_sjc_vnd"] = sjc_gap["gap"]
    if silver.get("rows"):
        top_silver = silver["rows"][0]
        data["bac_hang_dau"] = {
            "thuong_hieu": top_silver["brand"], "san_pham": top_silver["product"],
            "ban_ra": top_silver["sell"], "bien_dong_hom_nay": top_silver.get("change_24h"),
        }
    if big_moves:
        data["bien_dong_lon"] = [
            {"san_pham": m["label"], "giai_doan": m["period"], "phan_tram": round(m["pct"], 2)} for m in big_moves[:3]
        ]
    if extremes:
        data["cuc_tri_30_90_ngay"] = [
            {"san_pham": e["label"], "giai_doan": e["period"], "loai": e["kind"]} for e in extremes[:3]
        ]
    return data
...
def generate_commentary(data):
    """
    Ask Claude to write a short (3-4 sentence) Vietnamese market-
    commentary paragraph grounded strictly in `data` - the day's actual
    scraped figures, nothing invented. Returns the paragraph text, or
    None if ANTHROPIC_API_KEY isn't set or the call fails for any reason
    (network, auth, rate limit, etc.) - this is a nice-to-have layered on
    top of the email, never something that should block it from sending.
    """
    if not COMMENTARY_ENABLED or not data:
        return None
    prompt = (
        "Bạn là một nhà phân tích thị trường vàng bạc viết bản tin ngắn hàng ngày bằng tiếng Việt. "
        "Dựa CHỈ vào dữ liệu JSON dưới đây (không suy đoán, không thêm thông tin ngoài dữ liệu được cung cấp), "
        "viết một đoạn văn 3-4 câu tóm tắt diễn biến giá vàng và bạc hôm nay. "
        "Giọng văn chuyên nghiệp, súc tích, giống bản tin tài chính thực tế. "
        "Không dùng markdown, không tiêu đề, chỉ trả về đúng một đoạn văn thuần túy.\n\n"
    ...
    try:
        resp = requests.post(
            ...
            json={"model": COMMENTARY_MODEL, "max_tokens": 300, "messages": [{"role": "user", "content": prompt}]},
            timeout=30,
        )
        resp.raise_for_status()
        result = resp.json()
        ...
        print(f"  Commentary generation failed: {e}", file=sys.stderr)
        return None
...
def build_html(summary_tables, details, silver, silver_details, price_changes, world_gold, gap_rows,
                spreads, big_moves, has_chart, vcb_rate, extremes, portfolio, source_health, commentary_text,
    ...
    if commentary_text:
        commentary_html = f"""
            <tr><td style="padding:0 6px 16px;">
              <div class="dm-card" style="background:#fffaf0;border:1px solid #f0dfb8;border-radius:14px;padding:18px 20px;">
                <p style="margin:0 0 8px;font-size:11px;font-weight:700;color:{COLOR_GOLD};text-transform:uppercase;letter-spacing:0.05em;">✨ Nhận định thị trường (AI)</p>
                <p class="dm-text" style="margin:0;font-size:14.5px;line-height:1.6;color:{COLOR_TEXT};">{escape(commentary_text)}</p>
              </div>
            </td></tr>"""
...
def build_plain_text(summary_tables, details, silver, silver_details, price_changes, world_gold, gap_rows,
                      spreads, big_moves, has_chart, vcb_rate, extremes, portfolio, source_health, commentary_text,
                      source_url, timestamp):
    lines = [f"Gia vang & bac hom nay - cap nhat {timestamp}"]
    if commentary_text:
        lines.append("")
        lines.append("== NHAN DINH THI TRUONG (AI) ==")
...
def cmd_generate():
    ...
    if COMMENTARY_ENABLED:
        print(f"Generating AI commentary ({COMMENTARY_MODEL}) ...")
        commentary_input = build_commentary_input(summary_tables, silver, world_gold, gap_rows, big_moves, extremes)
        commentary_text = generate_commentary(commentary_input)
        print("Commentary: OK." if commentary_text else "Commentary: failed or empty, section will be omitted.")
    else:
    ...
    combined = {
        ...
        "vcb_rate": vcb_rate,
        "portfolio": portfolio,
        # commentary_text intentionally excluded from the dedup hash: it's
        # freshly generated prose every run (not a fact that "changed"),
        # so including it would make SEND_ONLY_ON_CHANGE=true never skip.
    }
    ...
    html_body = build_html(
        ...
        vcb_rate, extremes, portfolio, source_health, commentary_text, SOURCE_URL, timestamp,
    )
        ...
        json.dump(
            {
                ...
                "commentary_ok": bool(commentary_text),
